...
fitness <- function(V1, V2, eta, f = 0.1, g = 0.5, r0 = 0.5) {
    rho <- f * (1 - eta)
    Ni <- { rho / g } * exp({ r0 - rho } / rho)
    Vi <- cbind(V1, V2)
    C <- matrix(eta, 2, 2)
    diag(C) <- 1
...
# Points where fitness is maxed:
pt1 <- function(eta, f = 0.1, g = 0.5, r0 = 0.5) {
    rho <- f * (1 + eta)
    xy <- c(-1, 1) * sqrt((r0 - rho) / (2 * rho))
    tibble(V1 = xy, V2 = xy)
}
pt2 <- function(eta, f = 0.1, g = 0.5, r0 = 0.5) {
    rho <- f * (1 - eta)
    xy <- c(-1, 1) * sqrt((r0 - rho) / (2 * rho))
...
max_mag <- 6
eta_ <- -0.6
as_tibble(expand.grid(V1 = seq(-max_mag, max_mag, 0.1), 
                      V2 = seq(-max_mag, max_mag, 0.1))) %>% 
    mutate(fit = map2_dbl(V1, V2, ~ fitness(.x, .y, eta = eta_))) %>% 
    ggplot(aes(V1, V2)) +
    geom_raster(aes(fill = fit), interpolate = FALSE) +
    geom_point(data = bind_rows(pt1(eta_), pt2(eta_)), color = "white")
