...
one_eta_combo <- function(sign1, sign2, sign3) {
    ...
    trait_to$call[["eta"]] <- eval(trait_to$call[["eta"]])
...
#'   * All of the permutations with no sub-additive tradeoffs are stable
#'   * --+ is also stable
#'   * Everything else is unstable
#'
#' The state at - - + is the same as that for - 0/+ 0/+, but that one is
#' unstable
#'
...
all had high values of trait 2 and usually had high values of trait 3.
...
equaled zero.
With two super-additive and one additive combinations, species traits were 
along a neutrally stable quarter-ring along one axis.
With one super-additive and two additive combinations, species traits were 
...
of 1 sub-additive, 1 additive, and 1 super-additive trait (i.e., "1 of each"),
it allows for many possibilities:
anywhere on a sub-section of the neutral ring (like
the "2 super, 1 additive" scenario, but even more reduced).
...
to_cap <- "Figure 1.
           ...
           sub-additive ($\\eta < 0$ or \"$-$\"), additive ($\\eta = 0$), 
           ...
           that are sub-additive, additive, or super-additive.
           All but the \"all super\" and \"all additive\" sub-panels 
...
p_group_levels <- c("{} >= '2 sub'", "'1 sub'",
                    "'all super'",
                    "'2 super, 1 additive'", "'2 additive, 1 super'", 
                    "'all additive'")
