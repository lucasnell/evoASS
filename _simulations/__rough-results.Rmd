...
one_eta_combo <- function(sign1, sign2, sign3) {
    ...
    trait_to <- quant_gen(q = .q, eta = eta, d = 0.1, max_t = 20e3L, n_reps = 24,
...
In most cases, only one set of trait values were present in surviving species.


We observed alternative trait states only when no traits were sub-additive.
...
to_cap <- "Figure 1.
           ...
           Sub-panels separate the number of pairwise combinations that
           are sub-additive, neutral, or super-additive.
           All but the first three sub-panels contain multiple pairwise 
           combinations, so they are separated by color;
           each specific combination is indicated by the colored label.
           In the \"1 of each\" panel, each of the three points represents
           the resulting trait values for two combinations (i.e., two 
           combinations led to the same outcome).
...
```{r trait-outcomes-results, fig.cap = cc(to_cap), fig.width=6.5, fig.height=5.0}
p_group_levels <- c("all super", "all sub", "all neutral", "1 of each",
                    "2 super, 1 sub", "2 super, 1 neutral",
                    "2 sub, 1 super", "2 sub, 1 neutral",
...
# diag(eta) <- 1
# 
# trait_to <- quant_gen(q = .q, eta = eta, d = 0.9, max_t = 20e3L, n_reps = 24,
...
trait_outcomes_p_df %>% 
    filter(p_groups %in% c("1 of each", "2 super, 1 sub", "2 neutral, 1 sub")) %>% 
    filter(unq_spp_filter(V1, V2, V3))
...
if (.RESAVE_PLOTS) save_plot(trait_outcomes_p, 6.5, 5.0, "1-")
