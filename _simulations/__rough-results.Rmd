...
one_eta_combo <- function(sign1, sign2, sign3) {
    ...
    get_new_Vs <- function(.rep) {
        ...
        sigma2 <- with(list(n = 1), eval(formals(quant_gen)[["add_var"]]))^2
        deltaV <- sigma2 * sauron:::sel_str_cpp(V = V,
                                                N = N,
                                                ...
                                                D = D)
        V <- do.call(rbind, V) + deltaV
        
        return(V)
    }
    
    Vs <- lapply(1:24, get_new_Vs)
...
if (.REDO_SIMS) {
    ...
    saveRDS(eta_sims, rds("eta_sim_absolute_values"))
} else {
...
#'   * 
#'
eta_sim_eigs2 <- map_dfr(eta_sims,
                         function(.x) {
                             get_eigs <- function(j) {
                                 V <- as.numeric(.x[["new_traits"]][[j]])
                                 V <- ifelse(V < 0, -1, 1)
                                 J <- diag(V) %*% .x[["jacs"]][[j]]
                                 E <- max(eigen(J, only.values = TRUE)$values)
                                 return(E)
                             }
                             eigs <- map_dbl(1:length(.x[["new_traits"]]),
                                             get_eigs)
                             .x$ts %>%
                                 distinct(eta1, eta2, eta3) %>%
                                 mutate_all(sign) %>%
