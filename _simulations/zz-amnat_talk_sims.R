...
three_trait_pts_p2 <- three_trait_pts_p +
    geom_point(data = ts_q3_pe %>% prep_win_3d(), color = "dodgerblue")
...
ts_q3_pe %>%
    ...
    as.logical() %>%
    {prep_win_3d(ts_q3_pe)[.,]}


set.seed(7890557)
ts_q3_pe2 <- quant_gen(q = 3, eta = c(0.1, -0.1, 0.1), d = -1e-4, max_t = 5e3L, n_reps = 10,
...
ggplot(NULL, aes(V1, V2, size = V3)) +
    # geom_point(data = ts_q3_ne %>% prep_win_3d(),
    #            shape = 16, color = "firebrick3", alpha = 0.7) +
    geom_point(data = ts_q3_pe %>% prep_win_3d(),
               shape = 16, color = "dodgerblue", alpha = 0.7) +
    # geom_point(data = ts_q3_ze %>% prep_win_3d(),
