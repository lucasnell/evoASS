...
Below, $*$ is matrix multiplication
and
...
\begin{align}
    ...
    r(V_i) &= r_0 - f ~ V_i * C * V_i^{\text{T}} \\
    \alpha_{ii}(V_i) &= \alpha_0 ~\exp \left\{- V_i * V_i^{\text{T}} \right\} \\
    \alpha_{ij}(V_i, V_j) &= \alpha_0 ~\exp \left\{- V_i * V_i^{\text{T}} - d ~ V_j * V_j^{\text{T}} \right\} \\
    C &=
        \begin{pmatrix}
            1       & \eta  \\
            \eta    & 1     \\
...
$\alpha_0$ is the benefit of the trait on density dependence,
...
\begin{split}
F_{i} = \exp \{ 
    & ~ r_0 - f ~ V_i * C * V_i^{\text{T}} - \alpha_0 ~ N_i ~\text{e}^{- V_i * V_i^{\text{T}} } - \\
    & ~ \sum_{j \ne i}^{n}{ \alpha_0 ~ N_j ~\text{e}^{- V_i * V_i^{\text{T}} - d ~ V_j * V_j^{\text{T}} } } \}
\end{split}
\end{align}
$$
...
\begin{align}
F_{i} = \exp \left\{ 
        r_0 - f ~ V_i * C * V_i^{\text{T}} - \alpha_0 ~\text{e}^{ - V_i * V_i^{\text{T}} } 
        \left( ~ N_i + \sum_{j \ne i}^{n}{ N_j ~ \text{e}^{ - d ~ V_j * V_j^{\text{T}} } } ~ \right)
    \right\}
\end{align}
$$
...
\frac{\partial F_i}{\partial V_i} = \exp & \left\{
    ...
    \right\} \cdot \\
    & \quad \left[ 
        -f ~ V_i * (C + C^{\text{T}}) + 2 ~ \alpha_0 ~ V_i ~ \text{e}^{-V_i * V_i^{\text{T}}} 
        \left(
            N_i + \sum_{j \ne i}^{n}{N_j ~ \text{e}^{-d ~ V_j * V_j^{\text{T}}} }
        \right)
    \right]
\end{split}
\end{align}
