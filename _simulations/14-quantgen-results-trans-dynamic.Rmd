...
specifically over the period as the final species sets are determined.
The main question I'm trying to address here is whether the majority of 
trait changes typically occur before or after species go extinct.
...
get_sim <- function(eta_sign, d_sign, q) {
    fn <- sprintf("_output/quant_gen/q%i/eta=%s__d=%s.rds", q, eta_sign, d_sign)
    return(readRDS(fn))
}
...
## When are final species sets reached?


### From initial simulations:

For all scenarios, the final sets of species are reached very early:
...
# When each one reaches final set of species:
sim_df %>%
...
write_csv(max_times, "_output/quant_gen/max_times.csv")
```

```{r read-print-max_times, echo = FALSE}
max_times <- read_csv("_output/quant_gen/max_times.csv", col_types = "iddi")
print(max_times)
```
...
# This was done for the initial run so that I don't have to run the chunk above:
saveRDS(sim_df, "_output/quant_gen/short_sims.rds")
```

```{r read-sim_df-finest-scale, echo = FALSE}
sim_df <- readRDS("_output/quant_gen/short_sims.rds")
```
...
ggsave("_figures/transient_dynamics_q2.pdf", trans_plots2, width = 6.5, height = 9)
ggsave("_figures/transient_dynamics_q3.pdf", trans_plots3, width = 6.5, height = 9)
```
