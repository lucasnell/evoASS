...
N1 <- function(eta) {
    rho <- f * (1 + eta)
    (rho / a0) * exp((r0 - rho) / rho)
}
N2 <- function(eta) {
    rho <- f * (1 + eta)
    (rho / a0) * exp((r0 - rho) / (f * (1 - eta)))
}
cat(sprintf("%.10f\n%.10f\n", N1(0), N2(0)))
...
one_jacobian <- function(N, V, call_) {
    other_args <- c(as.list(call_)[c("eta", "d")],
                    with(formals(quant_gen), {
                        list(f = f, a0 = a0, add_var = eval(add_var))
                        }))
    args_ <- c(list(N = N, V = V),
               other_args[c("f", "a0", "d", "add_var")])
...
plot2_df <- map_dfr(1:length(sims2),
                  ~ sims2[[.x]]$nv %>%
...
#           legend.key = element_blank(), legend.margin = margin(0,0,0,0))
# ggsave("~/Desktop/plot2.png", ppt_plot2, width = 2.8, height = 3, dpi = 300)
...
plot3_df <- map_dfr(1:length(sims3),
                  ~ sims3[[.x]]$nv %>%
                      mutate(trait = factor(paste0("T", paste(trait)))) %>%
                      spread("trait", "value") %>%
...
#     theme(legend.position = "top", legend.direction = "horizontal",
#           legend.key = element_blank(), legend.margin = margin(0,0,0,0)) +
#     scale_alpha_manual(values = c(c(1, 0.1, 0.1), rep(c(1, 0.01, 0.01), 2)),
#                        guide = FALSE) +
#     scale_size_continuous("Trait 3", breaks = -1:1,
#                           guide = guide_legend(override.aes = list(color = "white")))
# ggsave("~/Desktop/plot3.png", ppt_plot3, width = 4, height = 4.25, dpi = 300)
...
#     theme(legend.position = "top", legend.direction = "horizontal",
#           legend.key = element_blank(), legend.margin = margin(0,0,0,0))
# ggsave("~/Desktop/plot3b.png", ppt_plot3b, width = 6, height = 4, dpi = 300)
```
