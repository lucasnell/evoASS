...
#'
#' `_add-mid` is for this being additive simulations with both
#' axes for the resident species starting at equal values.
#'
...
seeds <- c(1759901516, 1812523581, 1133429050, 11002500, 1482736070,
           1331237568, 1066452054)
...
one_sim_combo <- function(.d1, .sigma_V, .sigma_N) {
    ...
    # .sigma_V = 0.0; .sigma_N = 0.0; .d1 = -0.1
...
saveRDS(giant_sims, sprintf("giant_inv_sims_add-mid_%i.rds", i - 1L))
...
saveRDS(giant_sims, sprintf("giant_inv_sims_add-mid_outcomes_%i.rds", i - 1L))
