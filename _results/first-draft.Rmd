...
### NOTE ON KEEPING TRAITS NON-NEGATIVE:

Using absolute values to keep traits >= zero causes fluctuations near zero
...
This is why I'm not using absolute values.

The method I'm using changes traits to zero when they would otherwise go
negative.
One term for this is a "Heaviside step function" ($H(x)$ below).
The derivative of this function is the Dirac delta function ($\delta(x)$ below).
Below, $\mathbf{\ddot{V}}(\mathbf{V}_t)$ indicates the relationship between
$\mathbf{V}_{t+1}$ and $\mathbf{V}_t$ as show in the main text (i.e., without
any method to keep it from going negative).
Note also that below, I'm removing any indices for species for simplicity.
Here is a definition of the background equations that I've described:
...
Now here is how the main function and its derivative work when including the
...
derivative (the one that allows negatives) by the term
$\delta(\mathbf{\ddot{V}}(\mathbf{V}_t)) \mathbf{\ddot{V}}(\mathbf{V}_t) + H(\mathbf{\ddot{V}}(\mathbf{V}_t))$ to get the updated derivative.
Based on the definitions of $H(x)$ and $\delta(x)$...

* If $\mathbf{\ddot{V}}(\mathbf{V}_t)$ is 0 at equilibrium, then
$ \partial \mathbf{V}_{t+1} /  \partial \mathbf{V}_{t}$ is 
undefined (because you're multiplying $\infty$ by zero).
* If $\mathbf{\ddot{V}}(\mathbf{V}_t)$ is $> 0$ at equilibrium, then
