...
# Same but for a `quant_gen` object containing multiple reps:
check_diff <- function(qg_obj, time1, time2, precision = 1e-4) {
    time1 <- min(qg_obj$nv$time[qg_obj$nv$time > time1])
    if (missing(time2)) time2 <- max(qg_obj$nv$time)
    # Filtered data frame:
    fdf <- qg_obj$nv %>%
...
for (e in -1:1) {
    for (d in -1:1) {
        ...
        # 19.9e6 + 1, then 20e6
        if (d > 0) {
            z <- unlist(check_diff(qg, 19.9e6))
            if (any(z != 100)) {
                cat(sprintf("\n-------------\neta = %.2g, d = %.2g\n", e, d))
                print(z)
                print(unlist(check_diff(qg, 8e6)))
            }
        } else {
...
d <- 0
qg <- get_sim(e, d, q)
# For d <= 0, doesn't change after time == 10001
# For d > 0, traits don't change after 8e6+1
#     but abundances change until 19.9e6+1
...
#     geom_line(aes(group = spp, color = trait)) +
#     facet_wrap(~ rep, nrow = 10) +
#     NULL



sim_df2 <- map2_dfr(rep(1:3, each = 3), rep(1:3, 3), 
                    function(e, d, .q) {
                        chrs <- -1:1
                        qg_ <- get_sim(chrs[e], chrs[d], .q)
