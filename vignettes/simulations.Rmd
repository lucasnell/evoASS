...
filter_unq <- function(..., precision = 1e-2, min_pts = 100) {
    V <- do.call(cbind, list(...))
    if (nrow(V) <= min_pts) return(rep(TRUE, nrow(V)))
    V <- split(V, row(V))
...
match_trait <- function(trait, f_vec, trait_name = "T3") {
...
z <- q3 %>% 
    ...
    filter(eta != 0, stage %in% sprintf("Trait 3: %.2f", c(1.17, -1.17))) %>% 
    arrange(stage, eta, d) %>% 
    # group_by(stage, eta, d) %>%
    # filter((abs(T3) - t3[7])^2 == min((abs(T3) - t3[7])^2)) %>%
    # filter(1:n() == 1) %>%
    filter(filter_unq(T1, T2, T3, precision = 1e-1)) %>%
    # ungroup() %>%
    # group_by(eta, stage) %>% 
    # summarize_at(vars(starts_with("T")), mean) %>% 
    # ungroup() %>% 
    identity()

z %>% 
    filter(eta == -0.01, stage %in% sprintf("Trait 3: %.2f", -1.17)) %>%
    filter(filter_unq(T1, T2, T3, precision = 1e-1))
    
mean((c(0.415,-1.58,1.17) - c(-1.58,0.403,1.18))^2)
...
q3 %>% 
    ...
    mutate(stage = match_trait(T3, t3, "Trait 3")) %>% 
    group_by(eta) %>%
    filter(filter_unq(T1, T2, T3, precision = 1e-1, min_pts = 25)) %>%
    ungroup() %>%
    arrange(eta) %>% 
    ...
    xlab("Trait 1") +
    ylab("Trait 2") +
    scale_size_continuous("Trait 3", breaks = c(-1.5, -0.5)) +
    NULL
