...
Rcpp::sourceCpp("check_math/jacobian.cpp")

C <- matrix(qg$call$eta, q, q)
diag(C) <- 1

reps <- qg$nv %>% 
    filter(time == max(time)) %>% 
    group_by(rep, spp, N) %>% 
    summarize(traits = list(value)) %>% 
    ungroup() %>% 
    split(.$rep)

eigs_tony <- numeric(100)
for (repi in 1:100) {
    N <- reps[[repi]]$N
    V <- reps[[repi]]$traits
    jc_tony <- jacobian_cpp(V, N, qg$call$f, qg$call$g, C, qg$call$r0, qg$call$d, 
                        qg$call$add_var[1:length(N)], -1e-6)
    eigs_tony[repi] <- Re(eigen(jc_tony, only.values = TRUE, symmetric = FALSE)$values[1])
    if (repi %% 10 == 0) cat(repi, "\n")
}

range(eigs_tony - 1)

jc_sauron <- sauron:::jacobian_cpp(V, N, qg$call$f, qg$call$g,
                        qg$call$d, C, qg$call$add_var[1:length(N)])
eigen(jc_sauron, only.values = TRUE, symmetric = FALSE)$values[1]

new_sauron <- jc_sauron
for (i in 1:(length(N)-1)) {
    for (j in (i + 1):length(N)) {
        row_a <- (i-1) * q + 1
        col_a <- (j-1) * q + 1
        a <- new_sauron[row_a:(row_a + q - 1), col_a:(col_a + q - 1)]
        b <- new_sauron[col_a:(col_a + q - 1), row_a:(row_a + q - 1)]
        # Now assigning new values:
        new_sauron[col_a:(col_a + q - 1), row_a:(row_a + q - 1)] <- a
        new_sauron[row_a:(row_a + q - 1), col_a:(col_a + q - 1)] <- b
    }
}
eigen(new_sauron, only.values = TRUE, symmetric = FALSE)$values[1] - 1
eigen(jc_sauron, only.values = TRUE, symmetric = FALSE)$values[1] - 1
...
# How many species survived together?
qg %>%
    .[["nv"]] %>%
    filter(time == max(time)) %>%
    group_by(rep) %>%
