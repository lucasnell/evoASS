...
vignette: >
  %\VignetteIndexEntry{Competition model}
...
\begin{align}
    ...
    \alpha_{ii}(V_i) &= g ~\exp \left\{- V_i * V_i^{\text{T}} \right\} \\
    \alpha_{ij}(V_i, V_j) &= g ~\exp \left\{- V_i * V_i^{\text{T}} - d ~ V_j * V_j^{\text{T}} \right\} \\
    C &=
        \begin{pmatrix}
            1       & \eta  \\
            \eta    & 1     \\
...
\begin{split}
F_{i} = \exp \{ 
    & ~ r_0 - f ~ V_i * C * V_i^{\text{T}} - g ~ N_i ~\text{e}^{- V_i * V_i^{\text{T}} } - \\
    & ~ \sum_{j \ne i}^{n}{ g ~ N_j ~\text{e}^{- V_i * V_i^{\text{T}} - d ~ V_j * V_j^{\text{T}} } } \}
\end{split}
\end{align}
$$
...
\begin{align}
F_{i} = \exp \left\{ 
        r_0 - f ~ V_i * C * V_i^{\text{T}} - g ~\text{e}^{ - V_i * V_i^{\text{T}} } 
...
\frac{\partial F_i}{\partial V_i} = \exp & \left\{
    ...
    \right\} \cdot \\
    & \quad \left[ 
        -f ~ V_i * (C + C^{\text{T}}) + 2 ~ g ~ V_i ~ \text{e}^{-V_i * V_i^{\text{T}}} 
        \left(
