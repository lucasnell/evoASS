...
delta <- deltavec[delta.step]		
		...
		for (trial in 1:ntrials) {
			...
			for (time in 1:maxtime) {
				...
				if(T) if(time == maxtime/2){
					V <- matrix(exp(rnorm(n = n * q, mean = 0, sd = .3)), nrow = n, ncol = q) * V
					U <- matrix(exp(rnorm(n = p * q, mean = 0, sd = .3)), nrow = p, ncol = q) * U
				}
				A <- a + f * diag(V %*% C %*% t(V))
				B <- b * exp(-(V^2) %*% t(U^2))
				M <- m + g/diag(U %*% D %*% t(U))
	...
	##Calculated final fitnesses and selection pressure to see if we're at equilibrium		
			...
			WP <- exp(cc * t(B) %*% N - M)
			SV <- (-2 * r * f * sum(N) * V %*% C + 2 * r * b * V * exp(-(V^2) %*% t(U^2)) %*% (U^2 * 
			                                                                                     array(P, c(p, q))))
			SU <- (-2 * b * cc * U * exp(-(U^2) %*% t(V^2)) %*% (V^2 * array(N, c(n, q))) + 2 * g * 
			         array(1/diag(U %*% D %*% t(U))^2, c(p, q)) * (U %*% D))
			...
			if (traj.flag){
				...
				matplot(t(Nlist[, int * (1:(dim(Nlist)[2]/int))]), type = "l", ylab = "prey density")
				matplot(t(Plist[, int * (1:(dim(Plist)[2]/int))]), type = "l", ylab = "pred density")
				
				for (i in 1:q) {
					matplot(t(abs(Vlist[, i, int * (1:(dim(Vlist)[3]/int))])), type = "l", ylab = paste("prey trait", i))
					matplot(t(abs(Ulist[, i, int * (1:(dim(Ulist)[3]/int))])), type = "l", ylab = paste("pred trait", i))
				}
				dev.off()
			}
		
			##############################
			# truncate any values of V or U that are over 3 to 3 so that unique species can be identified
			V[V>3] <- 3
			U[U>3] <- 3
			##############################
      ...
      U[P < density.threshold,] <- 100		
			...
			if(num.prey > 0) {
				preylist <- rbind(preylist, matrix(c(array(trial, dim = num.prey), V[unique.prey, ]),nrow=num.prey))
			}else{
				preylist <- rbind(preylist, matrix(c(trial, array(NA, dim = q)),nrow=1))
			}
			predlist <- rbind(predlist, matrix(c(array(trial, dim = num.pred), U[unique.pred, ]),nrow=num.pred))
		...
		output.prey <- data.frame(prey.pred = "prey", delta=delta, etaN = etaN, etaP = etaP, rep=preylist[,1], tr1=preylist[,2], tr2=preylist[,3], tr3=preylist[,4], Fitness.N=Fitness.N, Fitness.P=Fitness.P, Selection.V=Selection.V,Selection.U=Selection.U)
		output.pred <- data.frame(prey.pred = "pred", delta=delta, etaN = etaN, etaP = etaP, rep=predlist[,1], tr1=predlist[,2], tr2=predlist[,3], tr3=predlist[,4], Fitness.N=Fitness.N, Fitness.P=Fitness.P, Selection.V=Selection.V,Selection.U=Selection.U)
		output <- rbind(output.prey, output.pred)
		if(delta.step ==1) {
			write.table(file="output coev instant etaN3_etaP2_22Oct18.csv", row.names = F, output, sep=',', col.names = T)
		}else{
			write.table(file="output coev instant etaN3_etaP2_22Oct18.csv", row.names = F, output, sep=',', col.names = F, append=T)
		}
		...
		for (i in 1:(ntrials - 1)) for (j in (i + 1):ntrials) {
			if (sc(matrix(preylist[preylist[, 1] == i, 2:4], ncol=3), matrix(preylist[preylist[, 1] == j, 2:4], ncol=3), precision = precision) == 
				T) 
		...
		for (i in 1:(ntrials - 1)) for (j in (i + 1):ntrials) {
			if (sc(matrix(predlist[predlist[, 1] == i, 2:4], ncol=3), matrix(predlist[predlist[, 1] == j, 2:4], ncol=3), precision = precision) == 
				T) 
		...
		output <- rbind(output.prey, output.pred)
		if(delta.step == 1) {
			write.table(file="output unique coev instant  etaN3_etaP2_22Oct18_multdelta.csv", row.names = F, output, sep=',', col.names = T)
		}else{
			write.table(file="output unique coev instant  etaN3_etaP2_22Oct18_multdelta.csv", row.names = F, output, sep=',', col.names = F, append=T)
		}
...
d <- read.table(file="output unique coev instant  etaN3_etaP2_22Oct18_multdelta.csv", sep=',', header = T)
summary(d)
...
# x <- prey.max.no
# image(matrix(x, nrow=length(etalist), ncol=length(etalist)), ylab=expression(eta[P]), xlab=expression(eta[N]), main = "prey", col=gray(level=(1:max(x))/max(x)), xaxt="n", yaxt="n", cex.lab=1.4)
# axis(side=1,at=c(0,.2,.4,.6,.8,1), labels=etalist)
# axis(side=2,at=c(0,.2,.4,.6,.8,1), labels=etalist)

# x <- pred.max.no
# image(matrix(x, nrow=length(etalist), ncol=length(etalist)), ylab=expression(eta[P]), xlab=expression(eta[N]), main = "pred", col=gray(level=(1:max(x))/max(x)), xaxt="n", yaxt="n", cex.lab=1.4)
# axis(side=1,at=c(0,.2,.4,.6,.8,1), labels=etalist)
# axis(side=2,at=c(0,.2,.4,.6,.8,1), labels=etalist)
