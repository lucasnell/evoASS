...
#include <vector>
#include <string>
#include "pcg_extras.hpp"  // pcg 128-bit integer type
#include "pcg_random.hpp"  // pcg prng
...
inline std::vector<std::vector<uint64_t>> mc_seeds(const uint32_t& n_cores) {
    ...
    std::vector<std::vector<uint64_t>> sub_seeds(n_cores, std::vector<uint64_t>(4));

    for (uint32_t i = 0; i < n_cores; i++) {
        sub_seeds[i] = as<std::vector<uint64_t>>(Rcpp::runif(4,0,4294967296));
    }
...
inline pcg32 seeded_pcg() {
    ...
    std::vector<uint64_t> sub_seeds = as<std::vector<uint64_t>>(Rcpp::runif(4,0,4294967296));
...
inline pcg32 seeded_pcg(const std::vector<uint64_t>& sub_seeds) {
...
// To sample for seeds before multi-core operations
inline std::vector<std::vector<uint64_t>> mc_seeds64(const uint32_t& n_cores) {
...
inline void fill_seeds64(const std::vector<uint64_t>& sub_seeds,
...
inline pcg64 seeded_pcg64() {
    ...
    std::vector<uint64_t> sub_seeds = as<std::vector<uint64_t>>(Rcpp::runif(8,0,4294967296));
    uint128_t seed1;
    uint128_t seed2;
...
inline pcg64 seeded_pcg64(const std::vector<uint64_t>& sub_seeds) {
...
// uniform in range [a,b]
inline uint32_t runif_aabb(pcg32& eng, const uint32_t& a, const uint32_t& b) {
