...
# for (trial in 1:ntrial) {
{
	...
	# for (time in 1:timeAdd) {
	{
		...
		if (time == timeAdd.long) {
			...
			FV <- V + sig2N * (-2 * r * f * sum(N) * V %*% C + 2 * r * b * V *
			                       exp(-(V^2) %*% t(U^2)) %*% (U^2 * array(P, c(p, q))))
			# Vt <- V + sig2N * (-2 * r * f * sum(N) * V %*% C + 2 * r * b * V *
			#                        exp(-(V^2) %*% t(U^2)) %*% (U^2 * array(P, c(p, q))))
			FU <- U + sig2P * (-2 * b * cc * U * exp(-(U^2) %*% t(V^2)) %*%
			                       (V^2 * array(N, c(n, q))) + 2 * g *
			                       array(1/diag(U %*% D %*% t(U))^2,
			                             c(p, q)) * (U %*% D))
			# Ut <- U + sig2P * (-2 * b * cc * U * exp(-(U^2) %*% t(V^2)) %*%
			#                        (V^2 * array(N, c(n, q))) + 2 * g *
			#                        array(1/diag(U %*% D %*% t(U))^2, c(p, q)) * (U %*% D))
			...
			for (i in 1:nn) {
				...
				FV1 <- V1 + sig2N * (-2 * r * f * sum(N) * V1 %*% C + 2 * r * b * V1 *
				                         exp(-(V1^2) %*% t(U1^2)) %*%
				                         (U1^2 * array(P, c(p, q))))
				# Vt <- V + sig2N * (-2 * r * f * sum(N) * V %*% C + 2 * r * b * V *
				#                        exp(-(V^2) %*% t(U^2)) %*% (U^2 * array(P, c(p, q))))
				FU1 <- U1 + sig2P * (-2 * b * cc * U1 * exp(-(U1^2) %*% t(V1^2)) %*%
				                         (V1^2 * array(N, c(n, q))) + 2 * g *
				                         array(1/diag(U1 %*% D %*% t(U1))^2, c(p, q)) *
				                         (U1 %*% D))
				# Ut <- U + sig2P * (-2 * b * cc * U * exp(-(U^2) %*% t(V^2)) %*%
				#                        (V^2 * array(N, c(n, q))) + 2 * g *
				#                        array(1/diag(U %*% D %*% t(U))^2, c(p, q)) * (U %*% D))
			...
			for (i in 1:n) {
			    HessianN[, , i] <- Jacobian[(q * (i - 1) + 1):(q * i),
			                                (q * (i - 1) + 1):(q * i)] -
			        diag(1, q, q)
			}
			HessianP <- array(0, dim = c(q, q, p))
			for (i in 1:p) {
			    HessianP[, , i] <- Jacobian[(n * q + q * (i - 1) + 1):(n * q + q * i),
			                                (n * q + q * (i - 1) + 1):(n * q + q * i)] -
			        diag(1, q, q)
			}
			print("")
			...
			print(paste("V Jacobian eigenvalue = ", max(abs(eigen(
			    Jacobian[1:(n * q), 1:(n * q)])$value))))
			...
			print(paste("U Jacobian eigenvalue = ", max(abs(eigen(
			    Jacobian[(1 + n * q):nn, (1 + n * q):nn])$value))))
		}
		################################################################
		...
		if (time == timeAdd.long) {
			...
			for(spN in 1:n){
				...
				for (ii in 0:100) for(jj in 0:100){
					...
					output$x[1] <- 1 * ii/100
					output$y[1] <- 1 * jj/100
					# write.table(output, file = "V.csv", sep = ",", append = T,
			...
			for(spP in 1:p){
				...
				for (ii in 0:100) for(jj in 0:100){
					...
					output$x[1] <- 2 * ii/100
					output$y[1] <- 2 * jj/100
					# write.table(output, file = "U.csv", sep = ",", append = T,
					#             col.names = F, row.names = F)
				}
			}
		}
...
#
# # resources
# plot(Vlist.full[1, 1, ], Vlist.full[1, 2, ], type = "n", ylab = "Resource trait 2",
#      xlim = c(0, 1), ylim = c(0, 1), xlab = "Resource trait 1",
# 	cex = cex, cex.lab = cex.lab)
# text(x = 0.005, y = 0.95, labels = "A. Resources", xpd = TRUE, adj = c(0, 0),
#      cex = cex.lab)
